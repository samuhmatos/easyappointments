...
the CalDAV integration of MarcaAgora.
...
- The right time zone value needs to be selected for synchronization to match the information MarcaAgora sends.
...
After making sure that the local Baikal server works, MarcaAgora will be able to connect to it and sync the
appointment data.
...
While trying to enable the CalDAV sync from the MarcaAgora calendar page, use the following credentials after
